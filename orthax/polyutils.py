...
def trimcoef(c, tol=0):
    ...
    Examples
    --------
    >>> from orthax import polyutils as pu
...
def getdomain(x):
    ...
    if jnp.iscomplexobj(x):
        ...
        return jnp.array(((rmin + 1j * imin), (rmax + 1j * imax)))
    else:
...
def _vander_nd(vander_fs, points, degrees):
    ...
    points = tuple(jnp.array(tuple(points), copy=False) + 0.0)
...
def _fromroots(line_f, mul_f, roots):
    ...
    def p_scan_fun(carry, x):
        return carry, _add(jnp.zeros(retlen, dtype=x.dtype), line_f(-x, 1))
...
def _fit(vander_f, x, y, deg, rcond=None, full=False, w=None):  # noqa:C901
    ...
    if issubclass(lhs.dtype.type, jnp.complexfloating):
        scl = jnp.sqrt((jnp.square(lhs.real) + jnp.square(lhs.imag)).sum(1))
    else:
