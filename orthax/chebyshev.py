...
def _zseries_mul(z1, z2, mode="full"):
    ...
    """
    return jnp.convolve(z1, z2, mode=mode)
...
def chebpow(c, pow, maxpower=16):
    ...
    if power != pow or power < 0:
        ...
        prd = jnp.zeros(len(c) * pow, dtype=c.dtype)
        prd = chebadd(prd, c)
...
def chebcompanion(c):
    ...
    scl = jnp.ones(n).at[1:].set(jnp.sqrt(0.5))
    shp = mat.shape
    mat = mat.flatten()
    mat = mat.at[1 :: n + 1].set(jnp.full(n - 1, 1 / 2).at[0].set(jnp.sqrt(0.5)))
    mat = mat.at[n :: n + 1].set(jnp.full(n - 1, 1 / 2).at[0].set(jnp.sqrt(0.5)))
    mat = mat.reshape(shp)
...
def chebpts1(npts):
    ...
    x = 0.5 * jnp.pi / _npts * jnp.arange(-_npts + 1, _npts + 1, 2)
    return jnp.sin(x)
...
class Chebyshev(pb.ABCPolyBase):
    ...
    # Virtual properties
    domain = jnp.array(chebdomain)
    window = jnp.array(chebdomain)
    basis_name = "T"
