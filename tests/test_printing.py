...
class TestRepr:
    def test_polynomial_repr(self):
        ...
        tgt = "Polynomial([0.0, 1.0], domain=[-1, 1], window=[-1, 1], symbol='x')"
        assert_equal(res, tgt)

    def test_chebyshev_repr(self):
        res = repr(poly.Chebyshev([0, 1]))
        tgt = "Chebyshev([0.0, 1.0], domain=[-1, 1], window=[-1, 1], " "symbol='x')"
        assert_equal(res, tgt)

    def test_legendre_repr(self):
        res = repr(poly.Legendre([0, 1]))
        tgt = "Legendre([0.0, 1.0], domain=[-1, 1], window=[-1, 1], " "symbol='x')"
        assert_equal(res, tgt)

    def test_hermite_repr(self):
        res = repr(poly.Hermite([0, 1]))
        tgt = "Hermite([0.0, 1.0], domain=[-1, 1], window=[-1, 1], " "symbol='x')"
        assert_equal(res, tgt)

    def test_hermiteE_repr(self):
        res = repr(poly.HermiteE([0, 1]))
        tgt = "HermiteE([0.0, 1.0], domain=[-1, 1], window=[-1, 1], " "symbol='x')"
        assert_equal(res, tgt)

    def test_laguerre_repr(self):
        res = repr(poly.Laguerre([0, 1]))
        tgt = "Laguerre([0.0, 1.0], domain=[0, 1], window=[0, 1], " "symbol='x')"
        assert_equal(res, tgt)
...
class TestLatexRepr:
    ...
    def as_latex(obj):
        ...
        obj._repr_latex_scalar = lambda x, parens=False: str(x.item())
        try:
