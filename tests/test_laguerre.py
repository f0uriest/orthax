...
class TestConstants:
    def test_lagdomain(self):
        assert_array_equal(lag.lagdomain, [0, 1])

    def test_lagzero(self):
        assert_array_equal(lag.lagzero, [0])

    def test_lagone(self):
        assert_array_equal(lag.lagone, [1])

    def test_lagx(self):
        assert_array_equal(lag.lagx, [1, -1])
...
class TestFitting:
    def test_lagfit(self):
        ...
        assert_array_almost_equal(lag.lagval(x, coef3), y)
        ...
        assert_array_almost_equal(lag.lagval(x, coef4), y)
...
class TestMisc:
    def test_lagfromroots(self):
        ...
        for i in range(1, 5):
            ...
            tgt = 0
            assert_(len(pol) == i + 1)
            assert_array_almost_equal(lag.lag2poly(pol)[-1], 1)
            assert_array_almost_equal(res, tgt)
    ...
    def test_poly2lag(self):
        for i in range(7):
            assert_array_almost_equal(lag.poly2lag(Llist[i]), [0] * i + [1])
