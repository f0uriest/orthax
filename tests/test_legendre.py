...
class TestConstants:
    def test_legdomain(self):
        assert_array_equal(leg.legdomain, [-1, 1])

    def test_legzero(self):
        assert_array_equal(leg.legzero, [0])

    def test_legone(self):
        assert_array_equal(leg.legone, [1])

    def test_legx(self):
        assert_array_equal(leg.legx, [0, 1])
...
class TestFitting:
    def test_legfit(self):
        ...
        assert_array_almost_equal(leg.legval(x, coef3), y)
        ...
        assert_array_almost_equal(leg.legval(x, coef4), y)
        ...
        assert_array_almost_equal(leg.legval(x, coef1), y)
        coef2 = leg.legfit(x, y, (0, 2, 4))
        assert_array_almost_equal(leg.legval(x, coef2), y)
...
class TestMisc:
    def test_legfromroots(self):
        ...
        for i in range(1, 5):
            ...
            tgt = 0
            assert_(len(pol) == i + 1)
            assert_array_almost_equal(leg.leg2poly(pol)[-1], 1)
            assert_array_almost_equal(res, tgt)
    ...
    def test_legline_zeroscl(self):
        assert_array_equal(trim(leg.legline(3, 0)), [3])
    ...
    def test_poly2leg(self):
        for i in range(10):
            assert_array_almost_equal(leg.poly2leg(Llist[i]), [0] * i + [1])
